<?php
include_once('./_common.php');
include_once("/phpqrcode/qrlib.php");

// 테마에 mypage.php 있으면 include
if(defined('G5_THEME_SHOP_PATH')) {
    $theme_mypage_file = G5_THEME_MSHOP_PATH.'/mypage.php';
    if(is_file($theme_mypage_file)) {
        include_once($theme_mypage_file);
        return;
        unset($theme_mypage_file);
    }
}

$g5['title'] = '마이페이지';
include_once(G5_MSHOP_PATH.'/_head.php');

// 쿠폰
$cp_count = 0;
$sql = " select cp_id
            from {$g5['g5_shop_coupon_table']}
            where mb_id IN ( '{$member['mb_id']}', '전체회원' )
              and cp_start <= '".G5_TIME_YMD."'
              and cp_end >= '".G5_TIME_YMD."' ";
$res = sql_query($sql);

for($k=0; $cp=sql_fetch_array($res); $k++) {
    if(!is_used_coupon($member['mb_id'], $cp['cp_id']))
        $cp_count++;
}
?>

<link rel="stylesheet" href="//code.jquery.com/ui/1.12.1/themes/base/jquery-ui.css">
<link rel="stylesheet" href="/resources/demos/style.css">
<script src="https://code.jquery.com/jquery-1.12.4.js"></script>
<script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script>
<script>
$( function() {
$( "#dialog" ).dialog({
    autoOpen: false,
    show: {
    effect: "blind",
    duration: 1000
    },
    hide: {
    effect: "explode",
    duration: 1000
    }
});

$( "#opener" ).on( "click", function() {
    $( "#dialog" ).dialog( "open" );
});
} );
</script>

<script>
    $(document).ready(function backand_get_balance() {
        var aaa = document.getElementById("ming1").innerHTML;

        $.ajax({
            type: 'POST',
            url: '/backand_get.php',
            data: {what: 'balance', parameter: aaa}
        }).done(function( msg ) {
            document.getElementById("ming3").innerHTML = msg + ` TA`
        });
    })
</script>

<style type="text/css">
.address {
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
    width: 120px;
    color:#0b66ff;
    right:20px;
}
.payment {
    color:#FF4848;
}
.starRating {
    color:#0b66ff;
}
</style>

<div id="smb_my">

    <section id="smb_my_ov">
        <h2>회원정보 개요</h2>
        <ul class="my_pocou">
            <br><br><a href="<?php echo G5_BBS_URL ?>/board.php?bo_table=notice">
                        <li class="my_cou">
                            <i class="fa fa-bullhorn" aria-hidden="true"></i> 공지사항 및 이벤트
                                <span><i class="fa fa-arrow-right" aria-hidden="true"></i></span>
                        </li>
                    </a>
            <br><li class="my_cou">
                        <i class="fa fa-list" aria-hidden="true"></i> 이용기록
                            <span><i class="fa fa-arrow-right" aria-hidden="true"></i></span>
                        </li>
            <?php if ($member[mb_id] && $member[mb_level] == 3) { ?>
            <br><br><li class="my_cou">
                        <i class="fa fa-car" aria-hidden="true"></i> 운행
                        </li>
            <br><li class="my_cou">
                        운행기록
                        <span><i class="fa fa-arrow-right" aria-hidden="true"></i></span>
                    </li>
            <br><li class="my_cou">
                        운행점수
                        <span class="starRating">
                            <i class="fa fa-star" aria-hidden="true"></i>
                            <i class="fa fa-star" aria-hidden="true"></i>
                            <i class="fa fa-star" aria-hidden="true"></i>
                            <i class="fa fa-star" aria-hidden="true"></i>
                            <i class="fa fa-star" aria-hidden="true"></i>
                        </span>
                    </li>
            <br>
            <?php } ?>
            <br><li class="my_cou">
                        <i class="fa fa-btc" aria-hidden="true"></i> 결제수단
                            <span class="payment">결제수단 등록이 필요합니다!</span>
                        </li>
            <br><br><li class="my_cou">
                        <i class="fa fa-th-large" aria-hidden="true"></i></i> 지갑주소
                        <?php if ($member['mb_1'] == NULL && $member['mb_2'] == NULL && $member['mb_3'] == NULL) { ?>
                            <span>타타타의 지갑을 만들어주세요!</span>
                        <?php } else { ?>
                            <span>
                                <div id="dialog" title="<?php echo $member['mb_nick'] ?> 님의 타타타 지갑주소 입니다!">
                                    <p><img src="https://chart.googleapis.com/chart?chs=250x250&cht=qr&chl=<?php echo $member['mb_1'] ?>"><br><?php echo $member['mb_1'] ?></p>
                                </div>
                                <i class="fa fa-qrcode" style="font-size:1.85em; cursor:pointer" id="opener"></i>
                                <!--<i class="fa fa-qrcode" style="font-size:1.85em; cursor:pointer" onclick="window.open('https://chart.googleapis.com/chart?chs=250x250&cht=qr&chl=<?php echo $member['mb_1'] ?>','asdfo8or','scrollbars=yes,width=417,height=385,top=10,left=20');"></i>-->
                                <p class="address"><?php echo $member['mb_1'] ?> </p>
                            </span> 
                        <?php } ?>
                        </li>
            <br><br><li class="my_cou">보유하신 타! 토큰
                            <span id="ming3"><?php echo number_format($cp_count); ?> TA</span>
                        </li>
            <br><li class="my_cou">보유하신 타! 쿠폰
                            <span><?php echo number_format($cp_count); ?> 장</span>
                        </li>
            <br><li class="my_point">보유포인트
                            <span><?php echo number_format($member['mb_point']); ?> 점</span>
                        </li>
            <br><br><a href="<?php echo G5_SHOP_URL; ?>/myshop.php">
                        <li class="my_cou"><i class="fa fa-shopping-basket" aria-hidden="true"></i><b> 타! 이용자들의 전용 SHOP</b>
                            <span><i class="fa fa-star" aria-hidden="true"></i></span>
                        </li>
            <?php if ($member[mb_id] && $member[mb_level] == 2) { ?>
            <br><br><a href="#">
                        <li class="my_cou">
                        <i class="fa fa-car" aria-hidden="true"></i> 드라이버 신청
                        </li>
                    </a>
                    <br>
            <?php } ?>
            <br><a href="<?php echo G5_BBS_URL ?>/member_confirm.php?url=register_form.php">
                        <li class="my_cou">
                            <i class="fa fa-cog" aria-hidden="true"></i> 내 설정
                        </li>
                    </a>
        </ul>
    </section>

    <script>
    
        $(".btn_op_area").on("click", function() {
            $(".my_info").toggle();
            $(".fa-caret-down").toggleClass("fa-caret-up")
        });

    </script>

    <!--
    <section id="smb_my_od">
        <h2>여름이니깐</h2>
        <?php
        // 최근 주문내역
        define("_ORDERINQUIRY_", true);

        $limit = " limit 0, 5 ";
        include G5_MSHOP_PATH.'/orderinquiry.sub.php';
        ?>
        <a href="<?php echo G5_SHOP_URL; ?>/orderinquiry.php" class="btn_more">더보기</a>
    </section>

    <section id="smb_my_od">
        <h2><a href="<?php echo G5_SHOP_URL; ?>/orderinquiry.php">최근 주문내역_mobile_shop_mypage</a></h2>
        <?php
        // 최근 주문내역
        define("_ORDERINQUIRY_", true);

        $limit = " limit 0, 5 ";
        include G5_MSHOP_PATH.'/orderinquiry.sub.php';
        ?>
        <a href="<?php echo G5_SHOP_URL; ?>/orderinquiry.php" class="btn_more">더보기</a>
    </section>

    <section id="smb_my_wish" class="wishlist">
        <h2><a href="<?php echo G5_SHOP_URL; ?>/wishlist.php">최근 위시리스트</a></h2>

        <ul>
            <?php
            $sql = " select *
                       from {$g5['g5_shop_wish_table']} a,
                            {$g5['g5_shop_item_table']} b
                      where a.mb_id = '{$member['mb_id']}'
                        and a.it_id  = b.it_id
                      order by a.wi_id desc
                      limit 0, 6 ";
            $result = sql_query($sql);
            for ($i=0; $row = sql_fetch_array($result); $i++)
            {
                $image_w = 250;
                $image_h = 250;
                $image = get_it_image($row['it_id'], $image_w, $image_h, true);
                $list_left_pad = $image_w + 10;
            ?>

            <li>
                <div class="wish_img"><?php echo $image; ?></div>
                <div class="wish_info">
                    <a href="./item.php?it_id=<?php echo $row['it_id']; ?>" class="info_link"><?php echo stripslashes($row['it_name']); ?></a>
                     <span class="info_date"><?php echo substr($row['wi_time'], 2, 8); ?></span>
                </div>
            </li>

            <?php
            }

            if ($i == 0)
                echo '<li class="empty_list">보관 내역이 없습니다.</li>';
            ?>
        </ul>
         <a href="<?php echo G5_SHOP_URL; ?>/wishlist.php" class="btn_more">더보기</a>
    </section>
    -->

</div>

<script>
$(function() {
    $(".win_coupon").click(function() {
        var new_win = window.open($(this).attr("href"), "win_coupon", "left=100,top=100,width=700, height=600, scrollbars=1");
        new_win.focus();
        return false;
    });
});

function member_leave()
{
    return confirm('정말 회원에서 탈퇴 하시겠습니까?')
}
</script>

<?php
include_once(G5_MSHOP_PATH.'/_tail.php');
?>