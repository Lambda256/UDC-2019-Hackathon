...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      valid_check: async function (productId) {
          ...
          try {
            ...
            if ((BigNumber(balanceResponse.data.data.balance).div((BigNumber('10')).pow(18)) >= parseInt(
              ...
              const cr = {
                method: "luni_areyou",
                params: {
                  randomStr: this.makeRandomStr(6)
                  // userAddress: this.walletAddress.userREOA
                }
              };
              // console.log(cr);  // eslint-disable-line
              this.sendCustomRequest(cr)
              .then(async response => {
                console.log("response - sendCustomRequest : ", response);  // eslint-disable-line

                const data = {
                  'from': this.accounts[0],
                  'inputs': {
                    "userAddress": this.accounts[0],
                    "randomStr": cr.params.randomStr
                  }
                };

                this.purchase(productId);
                // this.apiCheckIV(data)
                // .then(response => {
                //   console.log("response - apiCheckIV : ", response);  // eslint-disable-line
                //   if (response.data.data.res[0]) {
                //     this.purchase(productId);
                //   } else {
                //     alert('구매 불가능 본인이 아니십니다 !!!');
                //   }
                // });
              })
              .catch(error => {
      ...
      async purchase(productId) {
        try {
          await this.apiPurchase({
            'from': this.walletAddress.userREOA,
            'inputs': {
      ...
      async apiGetBalance() {
        return await this.axios.get(
            `https://api.luniverse.net/tx/v1.0/wallets/${this.walletAddress.userREOA}/${this.mtSymbol}/${this.stSymbol}/balance`, {
              headers: {
      ...
      async apiCheckIV(data) {
        return await this.axios.post(`https://api.luniverse.net/tx/v1.0/transactions/${this.txActionName.checkIV}`,
            data,
            {
              headers: {
